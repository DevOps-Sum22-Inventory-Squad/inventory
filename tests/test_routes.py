...
class TestInventoryServer(TestCase):
    ...
    def test_list_inventory_with_query(self):
        ...
        resp = self.client.get(BASE_URL + "?condition=NEW")
        data = resp.get_json()
    ...
    def test_delete_all_inventories(self):
        ...
        for i in range(3):
            resp = self.client.post(BASE_URL, json=target[i])
            self.assertEqual(resp.status_code, status.HTTP_201_CREATED)
    ...
    def test_list_inventory_with_query_illegal_value(self):
        ...
        resp = self.client.get(
            BASE_URL + "?quantity=ab&product_id=b&restock_level=c&condition=0")
